package com.haierac.biz.cp.cloudplatformandroid.utils.update;
...
import com.haierac.biz.cp.cloudplatformandroid.net.NetUtils;
import com.haierac.biz.cp.cloudplatformandroid.net.entity.UpdateResultBean;
import com.haierac.biz.cp.cloudplatformandroid.net.interfaces.RequestCallback;
import com.haierac.biz.cp.cloudplatformandroid.utils.LoggerUtils;
...
public class UpdateChecker extends Fragment {
    ...
    private void checkForUpdates(final String url) {
        mThread = new Thread() {
            @Override
            public void run() {
                NetUtils.requestFromUrl(NetUtils.URL_UPDATE, UpdateResultBean.class, new RequestCallback<UpdateResultBean>() {
                    @Override
                    public void onSuccess(UpdateResultBean target) {
                        LoggerUtils.e(TAG, "onSuccess: " + target.toString());
                        if ("10051".equals(target.getRetCode())) {
                            if (mTypeOfNotice == NOTICE_NOTIFICATION) {
                                showNotification(target.getUpdateContent(), NetUtils.BASE_URL + target.getUpdateUrl());
                            } else if (mTypeOfNotice == NOTICE_DIALOG) {
                                showDialog(target.getUpdateContent(), NetUtils.BASE_URL + target.getUpdateUrl());
                            }
                        }
...
//                CommonRestClient_.getInstance_(mContext).getAppVersion(LibConst.APP_ID, new RestClientCallback() {
//                    @Override
//                    public void success(HaierBaseResultBean entity) {
//                        GetAppVersionResultBean bean = (GetAppVersionResultBean) entity;
//                        int versionCode;
//                        try {
//                            versionCode = mContext.getPackageManager().getPackageInfo(mContext.getPackageName(), 0).versionCode;
//                            if (bean.version != null && Integer.valueOf(bean.version) > versionCode && isValidApkUrl(bean.resId)) {
//                                if (mTypeOfNotice == NOTICE_NOTIFICATION) {
//                                    showNotification(bean.description, bean.resId);
//                                } else if (mTypeOfNotice == NOTICE_DIALOG) {
//                                    showDialog(bean.description, bean.resId);
//                                }
//                            }
//
//                            if (bean.version != null && Integer.valueOf(bean.version) == versionCode) {
//                                //移除Notification
...
//
//                    @Override
//                    public void fail(HaierRestClientException exception) {
//
//                    }
//                });
            }
